...
def clean_df(
    ...
    df.loc[df["filename"].str.contains("timecourse_04"), "exp_type"] = df.loc[df["filename"].str.contains("timecourse_04"), "exp_type"].apply(lambda x: x + "_v3")
    ...
    for _subject, _date in exclude_pairs.items():
        df = df.loc[~((df["subject"] == _subject) & (df["start_time"].dt.floor("d") == _date))].copy()
    return df
